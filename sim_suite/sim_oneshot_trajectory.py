# Simulated One-Shot Trajectory Experiment Runner
...
class SimOneShotTrajectoryExperiment:
    """Runs a one-shot trajectory experiment with metrics tracking using simulation."""
    ...
    def run_experiment(self):
        ...
        print("\n" + "="*60)
        print("SIMULATED ONE-SHOT TRAJECTORY EXPERIMENT")
        print("="*60)
        ...
        while experiment_time < self.experiment_duration:
            ...
            if experiment_time >= next_control_time:
                ...
                # Update setpoint based on trajectory
                if self.trajectory and not self.trajectory.is_complete(experiment_time):
                    # Follow trajectory
                    trajectory_setpoint = self.trajectory.get_position(experiment_time)
                else:
                ...
                if len(self.time_log) % int(0.5 / self.control_dt) == 0:
                    traj_status = "following" if not self.trajectory.is_complete(experiment_time) else "complete"
        ...
        if len(self.time_log) > 0:
            ...
            print_metrics(metrics, "One-Shot Trajectory (Simulated)")
...
if __name__ == "__main__":
    ...
    experiment = SimOneShotTrajectoryExperiment(config_file=config_file, sim_config_file=sim_config_file)
