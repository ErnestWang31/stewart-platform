# Step PID Experiment Runner
# Runs a Step PID experiment: setpoint jumps from 10cm to 0cm at t=0
...
class StepPIDExperiment:
    ...
    def camera_thread(self):
        ...
        while self.running:
            ...
            cv2.imshow("Step PID Experiment - Ball Tracking", vis_frame)
    ...
    def run_experiment(self):
        ...
        # Set initial setpoint (10cm) - this is just for initial positioning
        self.pid.set_setpoint(self.initial_setpoint, 0.0)
        current_setpoint = self.initial_setpoint
        
        print("\n[EXPERIMENT] Starting...")
        print("[EXPERIMENT] Waiting for ball detection...")
        print(f"[EXPERIMENT] Initial setpoint: {self.initial_setpoint*100:.1f} cm")
        print(f"[EXPERIMENT] Target setpoint (at t=0): {self.target_setpoint*100:.1f} cm")
        
        # Wait for initial ball detection
        ...
        while self.running:
            try:
                ...
                if len(self.time_log) % 15 == 0:  # ~30 Hz, so every 0.5s
                    print(f"t={current_time:.2f}s | x={position_x*100:.2f}cm | r={current_setpoint*100:.2f}cm | "
        ...
        if len(self.time_log) > 0:
            ...
            print_metrics(metrics, "Step PID")
